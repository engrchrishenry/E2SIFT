...
# --- replace the old TSFNet_Chris class with this modified version ---
class TSFNet_Chris(nn.Module):
    def __init__(self, conv=default_conv):
        ...
        # register dct_min / dct_max as buffers so .to(device) moves them correctly
        self.register_buffer('dct_min', torch.from_numpy(np.load('/storage4tb/PycharmProjects/rpg_e2vid/dct_min_ssd_feat_recovery.npy')).float())
        self.register_buffer('dct_max', torch.from_numpy(np.load('/storage4tb/PycharmProjects/rpg_e2vid/dct_max_ssd_feat_recovery.npy')).float())
        ...
        # NEW: feature head that produces 512 channels (instead of pixel tail + pixelshuffle)
        self.feature_head = conv(dim_imgfeat, 512, kernel_size)

        # keep references if needed (old names removed)
        # self.pix_shuffle and old tail intentionally removed
    ...
    def forward(self, image):
        ...
        # produce dct representation using registered buffers
        ...
        res_pix += x_pix
        # print ('res_pix', res_pix.shape)
        
        # map to 512 channels and downsample spatially to (38, 38)
        feat = self.feature_head(res_pix)            # [B, 512, H_res, W_res] (H_res usually 80)
        # print ('feat', feat.shape)
        feat = F.adaptive_avg_pool2d(feat, (38, 38)) # [B, 512, 38, 38]
        # feat = self.sigmoid(feat)

        return feat

if __name__ == '__main__':
    vox = torch.rand(1, 5, 300, 300).cuda()
